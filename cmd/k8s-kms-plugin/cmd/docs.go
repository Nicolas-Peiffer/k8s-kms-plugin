...
func init() {
	...
	// Same reasoning as --output-dir above: this flag's real default is read from the
	// environment, so cobra rendered "(default true)" in CI and "(default false)" on a laptop —
	// a diff in two generated files that depended on where `make doc` ran, not on the CLI.
	// DefValue is only ever read to render help and documentation, never to resolve the flag.
	docsCmd.Flags().Lookup("provenance").DefValue = "true in GitHub Actions, false otherwise"
}
