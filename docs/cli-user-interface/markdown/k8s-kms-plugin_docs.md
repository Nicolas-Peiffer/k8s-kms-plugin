...
```
  ...
  -o, --output-dir string   Directory the generated files are written to. Defaults to a fresh timestamped directory under $TMPDIR, so two ad hoc runs never overwrite each other. (default "$TMPDIR/k8s-kms-plugin-docs-<timestamp>")
      --provenance          Stamp the build and CI run that produced the pages into the front matter of the generated markdown. Defaults to true when GITHUB_ACTIONS=true, so the published site records its build while the committed tree stays free of volatile data. (default true in GitHub Actions, false otherwise)
```
