...
reach the generated pages. Three things already had to be pinned down for that, and all three are
in `docs.go` or `root.go`:

- `--output-dir`'s default is a timestamped temp directory, so its `DefValue` is overridden with a
  `$TMPDIR/...<timestamp>` placeholder; without it the timestamp lands in two generated files on every run.
- `--provenance`'s default is read from `GITHUB_ACTIONS`, so its `DefValue` carries a placeholder too —
  otherwise the same tree generates "(default true)" in CI and "(default false)" on a laptop.
- `rootCmd` sets `DisableAutoGenTag`, which suppresses cobra's "Auto generated by spf13/cobra on <date>"
  footer. It stamped the run date into all twelve pages, so the CI freshness check (a plain `git diff`)
  failed on any day but the one they were last committed.
